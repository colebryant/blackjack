...
class GameGUI(ABC):
    ...
    def __init__(self, window):
        ...
        self._canvas = tk.Canvas(window, width=self._canvas_width,
...
class BlackJack(GameGUI):
    ...
    def __init__(self, window):
        ...
        self.text_font = tk.font.Font(family='Helvetica', size=15,
                                      weight='bold')
        Card.load_images()
        self.deck = Deck()
    ...
    def refresh_canvas(self):
        ...
        self._canvas.create_text(10, 10, anchor=tk.NW, fill='black',
                                 font=self.text_font,
                                 text=f'Player Hand Total:'
                                      f' {self.player_hand.total}')
        self._canvas.create_text(10, 150, anchor=tk.NW,
                                 font=self.text_font, fill='black',
                                 text=f'Dealer Hand Total:'
                                      f' {self.dealer_hand.total}')
        self._canvas.create_text(100, 300, anchor=tk.NW,
                                 fill=self.status_color,
                                 font=self.text_font,
                                 text=f'Game Status: {self.game_status}')
        self._canvas.create_text(10, 330, anchor=tk.NW, fill='black',
                                 font=self.text_font,
                                 text=f'Dealer Wins: {self.dealer_wins}')
        self._canvas.create_text(10, 355, anchor=tk.NW, fill='black',
                                 font=self.text_font,
